...
metadata {
	definition (name: "1.1.5 - Revised New Device Dimmer Driver",namespace: "jvm", author: "jvm") {
...
{	
	...
	{ ep, value ->
		...
		if (cd.is( null )) {
			...
			addChildDevice(value.driver.namespace, value.driver.type, childNetworkId, [name: "${device.displayName}-ep${ep}", isComponent: false])
		} 
	}
}
/////////////////////////////////////////////////////////////////
...
Map reparseDeviceData(deviceData = null )
{
	...
	// When data is stored in the state.deviceRecord it can lose its original data types, so need to restore after reading the data.
...
com.hubitat.app.DeviceWrapper getTargetDeviceByEndPoint(ep = null ) {
	if (ep) { return getChildDevices().find{ (it.deviceNetworkId.split("-ep")[-1] as Integer) == (ep as Integer)}
	} else { return device }
}
...
{ 
	log.debug "Received a NotificationSupportedReport: ${report}."
...
void zwaveEvent(hubitat.zwave.commands.notificationv8.NotificationReport cmd, ep = null )
{
	...
	if ( ! events ) { 
		...
		log.error "Device ${targetDevice.displayName}: Generated an Notification with an eventParametersLength > 1 from report ${cmd}." 
		}
	}
}
...
void	refreshBinarySensor(ep = null ) {
	log.debug "refreshBinarySensor function is not implemented."
}

void performBinarySensorRefresh(type, events, ep)
{
	log.debug "performBinarySensorRefresh function is not implemented."
...
void zwaveEvent(hubitat.zwave.commands.sensorbinaryv2.SensorBinaryReport cmd, ep = null )
{
	...
	if (logEnable) log.debug "Received SensorBinaryReport: ${cmd}"
...
void zwaveEvent(hubitat.zwave.commands.sensormultilevelv11.SensorMultilevelReport cmd, ep = null )
{
	...
	if (sensorReport) {
		...
		log.warn "Device ${device.displayName}: Received an unsupported sensor report: " + cmd
	}
}
...
[
	...
	[
		...
		// classVersions:[85:, 89:, 90:, 94:, 108:, 112:, 113:, 114:, 115:, 122:, 128:, 133:, 134:, 135:, 142:, 159: ], 
		endpoints:[
				0:[
		...
		deviceInputs:[
			...
			3:[ size:1,	category:"advanced", name:"3", title:"(3) App Level Retry Base Wait", description:"Application Level Retry Base Wait Time Period", range:"1..96", type:"number"],
	...
	[
		...
		endpoints:[0:[classes:[0, 32, 37, 39, 43, 44, 89, 90, 91, 94, 112, 114, 115, 122, 133, 134]]], 
		fingerprints: [
	...
	[
		...
		endpoints:[0:[classes:[0, 32, 38, 39, 43, 44, 89, 90, 91, 94, 112, 114, 115, 122, 133, 134]]], 
		deviceInputs:[
	...
	[
		...
		endpoints:[
			0:[classes:[0, 32, 37, 43, 44, 50, 89, 90, 91, 94, 112, 114, 115, 122, 133, 134], metersSupported:[0, 2, 4, 5, 6]]
			], 
		deviceInputs:[
	...
	[
		...
		endpoints:[ // List of classes for each endpoint. Key classes:  0x25 (37) or 0x26 (38), 0x32 (50), 0x70  (switch, dimmer, metering, 
							...
							1:[driver:[type:"Generic Component Dimmer", namespace:"hubitat"], classes:[0x26] ], 
							2:[driver:[type:"Generic Component Fan Control", namespace:"hubitat"], classes:[0x26] ]
				],
		deviceInputs:[
	...
	[
		...
		endpoints:[
					...
					1:['driver':[type:'Generic Component Switch', 'namespace':'hubitat'], classes:[32, 37, 89, 94, 133, 142]]
					], 
		deviceInputs:[
