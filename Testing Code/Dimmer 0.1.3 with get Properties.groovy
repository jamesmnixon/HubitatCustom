...
metadata {
	definition (name: "[Beta 0.1.3 - with getProps] Almost Any Dimmer Z-wave Plus Dimmer Driver",namespace: "jvm", author: "jvm") {
...
hubitat.zwave.Command indicatorSupportedGet(Short indicatorId)
{
	
	sendToDevice(secure(zwave.indicatorV3.indicatorSupportedGet(indicatorId: indicatorId)))
	return myReportQueue("8705").poll(10, TimeUnit.SECONDS)
}
...
void zwaveEvent(hubitat.zwave.commands.indicatorv3.IndicatorSupportedReport cmd, Short ep = null ) {
...
hubitat.zwave.Command  getCachedZwaveplusInfoReport(Short ep = null ) { 
								if (! implementsZwaveClass(0x5E, ep)) return null
								getReportCachedByProductId(zwave.zwaveplusInfoV2.zwaveplusInfoGet(), ep) 
							}
// =============================================
...
void zwaveEvent(hubitat.zwave.commands.zwaveplusinfov2.ZwaveplusInfoReport cmd, Short ep = null )  									{ transferReport(cmd, ep) }
...
void zwaveEvent(hubitat.zwave.commands.supervisionv1.SupervisionGet cmd, Short ep = null ) {
	...
	log.debug "Device ${targetDevice.displayName}: Received a SupervisionGet. Encapsulated command was: ${encapsulatedCommand}"
